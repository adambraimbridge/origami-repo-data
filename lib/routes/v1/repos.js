...
module.exports = app => {
	...
	app.get('/v1/repos', requireAuth(requireAuth.READ), cacheForFiveMinutes, async (request, response, next) => {
		...
		try {
			let repos = (await app.model.Version.fetchRepos()).map(version => {
				return version.serializeAsRepo();
			});

			// List repositories by brand
			if (brand) {
				// List all branded repositories
				if (brand === 'all') {
					repos = repos.filter(repo => repo.brands);
				// List all unbranded repositories
				} else if (brand === 'none') {
					repos = repos.filter(repo => !repo.brands);
				// List repositories by specific brand
				} else {
					repos = repos.filter(repo => repo.brands && repo.brands.includes(brand));
				}
			}
